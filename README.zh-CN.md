...
### 从 Codex 插件 marketplace 安装

从本仓库把 Gearshift 安装到 Codex：
...
然后重启 Codex 或开启新会话，让插件 Skills 和命令生效。

### 从本地 checkout 安装

开发 Gearshift 或测试未发布改动时，可以使用本地 checkout：
...
在每个需要 Gearshift 路由的仓库里运行：

```text
$gearshift:init
```

初始化只会把 Gearshift 托管说明写入 `AGENTS.md` / `CLAUDE.md` 的 managed block。它不会创建 `.gear` 目录、task 文件或工作流文档。
...
Doctor 会检查插件版本、Complex 模式所需的兼容 `grill-me` / `grill-with-docs`、托管说明和路由 fixtures。
